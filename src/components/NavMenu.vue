...
<script>
import * as seleData from "../utils/dataInfor.js"; // 导入图表的数据
...
export default {
  data() {
    return {
      ...
      clickChart: -1,
      choseCharts: seleData.default.choseCharts, //图表类型
      choseCWhichOne: "", //选择图表
      selectChartType: "", //详细图表
      chartTie: "", //图表名称
      imgArr: [],
      chartChoseW: seleData.default.chartChoseW, // 图表的宽度
      wChoseWhichOne: "", //选择的宽度
      chartChoseH: seleData.default.chartChoseH, // 图表的高
        ...
        if (this.selectChartType === "doubleLine") {
          for (var j = 0; j < result.data.seriesData.length; j++) {
            for (var x = 0; x < this.colorArr.length; x++) {
              if (j === x) {
                doubleArr.push({
                  data: result.data.seriesData[j].data,
